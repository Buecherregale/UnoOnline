 ...
 */
export function createApiErrorMessage(statusCode: number, originalMessage?: string): string {
  ...
  return errorMessages[statusCode] || originalMessage || 'Ein unerwarteter Fehler ist aufgetreten.'
}
 ...
 */
export function validatePlayerSession(player: any): asserts player is { id: string; name: string } {
  if (!player?.id) {
    throw new Error('Spieler nicht in der Sitzung gefunden. Bitte melden Sie sich erneut an.')
  }
}
